...
const int ENCODER_PULSE_PER_REVOLUTION = 21;  // Number of encoder pulses per revolution
...
volatile int16_t encoder_counts = 0;
...
void setup() {
  ...
  //Set a second set of encoder pins as input with pull-up resistors
...
void loop() {
  ...
  if(encoderALast != ecoderAState && encoderBLast != encoderBState){
    encoder_isr();
  }
  else{
    encoder_counts = 0;
  ...
  if(encoderALast != encoderAState || encoderBLast != encoderBState){
    encoder_sir();
  }
  // Print results
